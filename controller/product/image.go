...
func (ic *ImageController) UploadProductImage(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, http.StatusBadRequest, 400, "请上传有效的图片文件")
		return
	}
	defer file.Close()
	...
	response.Success(c, http.StatusCreated, imageDTO)
}
...
func (ic *ImageController) SetPrimaryImage(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, http.StatusBadRequest, 400, "无效的图片ID")
		return
	}
	...
	response.Success(c, http.StatusOK, gin.H{"message": "设置主图成功"})
}
...
func (ic *ImageController) UpdateImageSortOrder(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.Error(c, http.StatusBadRequest, 400, "请提供有效的排序序号（正整数）")
		return
	}
	...
	response.Success(c, http.StatusOK, gin.H{"message": "更新排序成功"})
}
...
func (ic *ImageController) DeleteProductImage(c *gin.Context) {
	...
	response.Success(c, http.StatusOK, gin.H{"message": "删除图片成功"})
}