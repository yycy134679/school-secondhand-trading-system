...
import (
	...
	"school-secondhand-trading-system/service/tag"
...
func (tc *TagController) ListTags(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, http.StatusInternalServerError, 500, "获取标签列表失败: "+err.Error())
		return
	}
...
func (tc *TagController) CreateTag(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, http.StatusInternalServerError, 500, "创建标签失败: "+err.Error())
		return
	}
...
func (tc *TagController) UpdateTag(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, http.StatusBadRequest, 400, "无效的标签ID")
		return
	}
	...
	if err != nil {
		response.Error(c, http.StatusInternalServerError, 500, "更新标签失败: "+err.Error())
		return
	}
...
func (tc *TagController) DeleteTag(c *gin.Context) {
	...
	if err != nil {
		...
		if err == tag.ErrTagHasProducts {
			response.Error(c, http.StatusBadRequest, tag.ErrCodeTagHasProducts, err.Error())
			return
		}
		response.Error(c, http.StatusInternalServerError, 500, "删除标签失败: "+err.Error())
		return
	}
