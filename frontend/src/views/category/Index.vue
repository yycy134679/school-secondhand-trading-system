<template>
  <div class="category-page">
    ...
    <div class="category-header">
      ...
      <p class="category-description">浏览 {{ categoryName }} 分类下的所有商品</p>
    </div>
...
const categoryName = computed(() => {
  const category = appStore.categories.find((c) => c.id === categoryId.value)
...
const updateRoute = () => {
  ...
  router.push({ query, params: { id: String(categoryId.value) } })
}
...
const loadProducts = async () => {
  ...
  try {
    ...
    const result = await productStore.fetchCategoryProducts(categoryId.value, params)
...
watch(
  ...
  ([newId, newQuery]) => {
    ...
    // 同步筛选条件
    filters.value = {
      sort: (query.sort as string) || 'latest',
      minPrice: query.minPrice ? Number(query.minPrice) : undefined,
      maxPrice: query.maxPrice ? Number(query.maxPrice) : undefined,
      conditionIds: query.conditionIds ? String(query.conditionIds) : '',
    }
