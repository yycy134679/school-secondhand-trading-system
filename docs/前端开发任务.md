...
* [x] 4. 学生端通用组件开发（frontend/src/components）
  ...
  * [x] 4.4 用户相关组件（`components/user/`）

    * [x] 4.4.1 `LoginModal.vue`

      * [x] 4.4.1.1 实现账号、密码、记住我、登录按钮，调用 `userStore.login` 对接 `POST /users/login`。
      * [x] 4.4.1.2 登录成功后关闭 Modal，刷新当前路由（或重定向到之前受保护的路由）。
    * [x] 4.4.2 `RegisterModal.vue`

      * [x] 4.4.2.1 实现账号、昵称、密码、确认密码，调用 `POST /users/register`；可选输入微信号（提前填）。
      * [x] 4.4.2.2 注册成功后保存 token + 用户信息并关闭 Modal，跳转首页。
    * [x] 4.4.3 `AvatarUpload.vue`

      * [x] 4.4.3.1 支持选择/拖拽上传单张头像图，在上传成功后拿到 URL 并调用 `PUT /users/profile` 更新 `avatarUrl`。
...
* [ ] 5. 学生端关键页面开发与联调（按页面拆解）
  ...
  * [ ] 5.1 首页 `/`（`frontend/src/views/home/Index.vue`）
    ...
    * [x] 5.1.1 布局与结构
      ...
      * [x] 5.1.1.1 上方 Hero 区：简短欢迎语 + 热门搜索词 Tag（点击跳转到 `/search?q=xxx`）。
      * [x] 5.1.1.2 分类导航条：从 `appStore.categories` 渲染横向滚动分类按钮，点击跳转 `/category/:id`。
      * [x] 5.1.1.3 两个列表区："为你推荐"（登录且有推荐时显示）。
    * [x] 5.1.2 状态管理与数据加载

      * [x] 5.1.2.1 进入页面时调用 `productStore.fetchHomeData({page, pageSize})`，对接 `GET /home`。
      * [x] 5.1.2.2 将响应中的 `data.recommendations` 存入 `homeRecommendations`，`data.latest` 分页信息存入 `homeLatest`。
    * [x] 5.1.3 接口调用与骨架屏

      * [x] 5.1.3.1 在加载阶段展示商品卡片骨架屏（若列表为空且 loading 为 true），避免大面积 spinner。
    * [x] 5.1.4 交互与空状态

      * [x] 5.1.4.1 当 `recommendations` 为空或用户未登录时隐藏"为你推荐"模块。

  * [x] 5.2 搜索页 `/search`（`frontend/src/views/search/Index.vue`）

    * [x] 5.2.1 布局与过滤条

      * [x] 5.2.1.1 从路由 query 读取 `q` 初始值，顶部显示搜索关键词。
      * [x] 5.2.1.2 实现筛选栏（吸顶）：排序（最新/价格升/降）、价格区间、新旧程度、发布时间（全部/近7天/近30天）。
    * [x] 5.2.2 API 对接

      * [x] 5.2.2.1 调用 `GET /products/search`，将以上过滤条件映射为 `q`、`categoryId`、`tagIds`、`conditionIds`、`minPrice`/`maxPrice`、`publishedTimeRange`、`sort`、`page/pageSize`。
      * [x] 5.2.2.2 将返回的 `items` 通过 `ProductCard` 渲染网格列表。
    * [x] 5.2.3 交互与状态

      * [x] 5.2.3.1 任何筛选条件/排序变更时，更新路由 query 并重新请求搜索接口。
      * [x] 5.2.3.2 如果无结果，展示 `Empty`，文案类似“没有找到相关商品，换个词试试？”。

  * [x] 5.3 分类页 `/category/:id`（`frontend/src/views/category/Index.vue`）

    * [x] 5.3.1 从路由读取分类 ID，在顶部显示分类名称（从 `appStore.categories` 查找）。
    * [x] 5.3.2 调用 `GET /products/category/{categoryId}`，支持 `minPrice`/`maxPrice`/`sort`/分页。
    * [x] 5.3.3 列表与分页使用 `ProductCard` + `Pagination`，空列表时展示 `Empty`。

  * [x] 5.4 商品详情页 `/products/:id`（`frontend/src/views/product/Detail.vue`）

    * [x] 5.4.1 布局

      * [x] 5.4.1.1 左侧图片区域：主图 + 缩略图列表，点击缩略图切换主图；支持点击放大为 Lightbox。（前端可以先实现基础切换，Lightbox 可作为增强）
      * [x] 5.4.1.2 右侧信息区：标题、价格、分类标签、新旧程度 Tag、发布时间、卖家卡片、描述、操作按钮区域。
    * [x] 5.4.2 数据加载

      * [x] 5.4.2.1 调用 `GET /products/{id}` 获取 `ProductDetail`，包括 `images`、`status`、`seller`、`viewerIsSeller` 等。
      * [x] 5.4.2.2 加载中展示骨架屏；加载失败显示错误提示。
    * [x] 5.4.3 状态与操作控制

      * [x] 5.4.3.1 若 `status` 为 `Sold` 或 `Delisted`，在顶部显示灰色横幅"该商品已售出/已下架"，禁用"联系卖家"按钮。
      * [x] 5.4.3.2 若 `viewerIsSeller` 为 true，则不显示"联系卖家"，替换为"编辑商品"按钮跳转 `/products/:id/edit`。
    * [x] 5.4.4 联系卖家交互

      * [x] 5.4.4.1 未登录点击"联系卖家"时弹出 `LoginModal`，登录成功后返回详情页并自动尝试再次调用 contact 接口。
      * [x] 5.4.4.2 已登录且非卖家：调用 `GET /products/{id}/contact`，根据返回：

        * [x] 5.4.4.2.1 `canContact=true` 且 `sellerWechat` 非空：展示微信号弹窗，可一键复制，并显示"请线下交易，注意安全"。
        * [x] 5.4.4.2.2 `canContact=false` 且 `tips` 存在（如微信号为空）：展示降级提示"卖家联系方式暂不可用，请稍后再试"。

  * [x] 5.5 发布商品页 `/products/new`(`frontend/src/views/product/New.vue`)

    * [x] 5.5.1 访问控制:路由 `meta.requiresAuth=true`,未登录时路由守卫触发登录。
    * [x] 5.5.2 使用 `ProductForm` 组件作为主内容,加载 `appStore.categories`、`tags`、`productConditions` 作为选项。
    * [x] 5.5.3 在提交前检查当前用户 `wechatId` 是否为空:若为空,提示用户前往个人中心填写后再发布(或在底部提供微信号输入并调用 `PUT /users/profile` 补齐)。
    * [x] 5.5.4 提交逻辑:

      * [x] 5.5.4.1 将表单数据转为 `FormData`,含 `title`、`description`、`price`、`categoryId`、`tagIds` 字符串、`conditionId`、`images` 数组。
      * [x] 5.5.4.2 调用 `POST /products` 成功后,跳转到新建商品详情页或"我发布的商品"。
      * [x] 5.5.4.3 提交期间禁用按钮并显示 loading 状态,防止重复提交。

  * [x] 5.6 编辑商品页 `/products/:id/edit`（`frontend/src/views/product/Edit.vue`）

    * [x] 5.6.1 初始化加载：根据 `:id` 调用 `GET /products/{id}`，并校验当前用户是否为 `seller`（若不是，跳回首页或展示无权限）。
    * [x] 5.6.2 若商品状态为 `Sold`，前端在 UI 上直接展示"已售商品不能编辑"的提示，同时让提交按钮不可用（尽管后端也会用 `3004` 拦截，前端先行友好提示）。
    * [x] 5.6.3 将初始数据传入 `ProductForm` 的 `initialValue`；提交调用 `PUT /products/{id}`，字段与创建时对应（只传需要变更字段）。

  * [x] 5.7 我发布的商品 `/my/products`（`frontend/src/views/product/MyProducts.vue`）

    * [x] 5.7.1 页面结构：列表 + 状态标签 + 操作按钮区，下方分页。
    * [x] 5.7.2 数据加载：调用 `GET /products/my?keyword=&page=&pageSize=`，展示 `mainImageUrl`、标题、价格、状态、发布时间。
    * [x] 5.7.3 操作按钮：

      * [x] 5.7.3.1 对 `ForSale`：可"编辑"、"下架"、"标记已售"。
      * [x] 5.7.3.2 对 `Delisted`：可"编辑"、"重新上架"。
      * [x] 5.7.3.3 对 `Sold`：仅显示"已售出"标签，不提供状态变更按钮。
    * [x] 5.7.4 上下架与标记已售交互：

      * [x] 5.7.4.1 调用 `POST /products/{id}/status`，传 `action` 为 `delist`/`relist`/`sold`。根据返回状态更新列表。
      * [x] 5.7.4.2 对 `delist` 与 `relist` 操作，在成功后展示 3 秒内可点击"撤销"的横幅，撤销时调用 `POST /products/{id}/status/undo`；根据 `3005` 错误码展示"撤销超时或状态已改变"。

  * [x] 5.8 个人中心 `/profile`（`frontend/src/views/user/Profile.vue`）

    * [x] 5.8.1 页面结构：Tab 或垂直菜单，包含"个人资料"、"我发布的"、"最近浏览"。
    * [x] 5.8.2 个人资料 Tab：

      * [x] 5.8.2.1 显示头像、昵称、账号、微信号。昵称与微信号支持编辑：调用 `PUT /users/profile` 更新，昵称修改需处理 `1001` 频率限制错误。
      * [x] 5.8.2.2 嵌入 `AvatarUpload` 用于头像修改。
    * [x] 5.8.3 修改密码 Tab：

      * [x] 5.8.3.1 表单包含旧密码、新密码、确认新密码，调用 `PUT /users/password`。
      * [x] 5.8.3.2 对 `1001` 错误返回"旧密码不正确"或"新密码不满足要求"提示。
    * [x] 5.8.4 最近浏览 Tab：

      * [x] 5.8.4.1 调用 `GET /users/recent-views`，渲染最近浏览商品卡片，最多 20 条按时间倒序。

  * [x] 5.9 登录/注册页（备用）`/login`、`/register`（`frontend/src/views/user/Login.vue` & `Register.vue`）

    * [x] 5.9.1 复用 Modal 内逻辑，提供全页形式的登录/注册页面（与弹窗 UI 统一）。
    * [x] 5.9.2 登录成功后按 PRD 要求跳转首页；注册成功同样自动登录并进入首页。
...
* [ ] 6. 学生端联调、自测与验收要点
  ...
  * [x] 6.1 接口联调检查
    ...
    * [x] 6.1.1 针对所有 API，使用浏览器网络面板/接口调试工具核对 HTTP 方法、URL、参数名与 `api.md` 一致，尤其是 `tagIds` 字符串格式、`conditionId`、分页参数。
    * [x] 6.1.2 校验状态机相关接口：非法状态流转应返回 `3003` 或 `3004`，前端适配文案。
