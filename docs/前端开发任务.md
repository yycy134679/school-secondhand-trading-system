...
* [ ] 2. 学生端（frontend/）功能模块梳理与路由/接口对齐
  ...
  * [ ] 2.2 路由结构与权限标记（`frontend/src/router/index.ts`）

    * [ ] 2.2.1 定义公开路由：`/`（首页）、`/search`、`/category/:id`、`/products/:id`、`/login`、`/register`，与 IA 一致。
    * [ ] 2.2.2 定义需登录的路由：`/products/new`、`/products/:id/edit`、`/my/products`、`/profile`，通过 `meta.requiresAuth = true` 标记。
    * [ ] 2.2.3 在路由守卫中使用 Pinia `user` store：若访问 `requiresAuth` 路由但未登录，则触发登录弹窗（或跳转 `/login`，再回跳）。
    * [ ] 2.2.4 根据路由 meta 配置页面标题（document.title）与面包屑信息（供后期使用）。
...
* [ ] 3. 学生端全局基础设施搭建（frontend/src）

  * [ ] 3.1 入口与根组件

    * [ ] 3.1.1 在 `frontend/src/main.ts` 中创建应用实例，注册 Router 与 Pinia，并挂载到 `#app`。
    * [ ] 3.1.2 在 `frontend/src/App.vue` 中实现基础布局：顶部为 `AppHeader`，底部为 `AppFooter`，中间为一个宽度 1200px 的居中容器包裹 `<RouterView />`，满足 Modern Minimalism 布局规范。

  * [ ] 3.2 全局样式与主题（Modern Minimalism）

    * [ ] 3.2.1 在 `frontend/src/assets/styles/reset.css` 中引入 CSS Reset（可用现代 reset 或自定义），在 `main.ts` 全局引入。
    * [ ] 3.2.2 在 `common.scss` 中定义 spacing（4px 基准）、字体、断点、常用 flex 工具类、按钮样式基础变量。
    * [ ] 3.2.3 在 `theme.scss` 中定义主要颜色：主色 `#0066FF`、中性色、功能色（成功/错误/警告），并暴露 CSS 变量，供组件引用。
    * [ ] 3.2.4 统一定义字体 `font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Helvetica Neue', sans-serif;`，价格字体加粗并放大 2px。

  * [ ] 3.3 工具与请求封装（`frontend/src/utils/` 与 `api/`）

    * [ ] 3.3.1 在 `utils/request.ts` 中创建 axios 实例，`baseURL` 从 `import.meta.env.VITE_API_BASE_URL` 读取，设置超时、`Content-Type` 默认值等。
    * [ ] 3.3.2 配置请求拦截器：从 `store/user` 或 `auth.ts` 读取 token，在所有需要登录的请求上添加 `Authorization: Bearer <token>` 头。
    * [ ] 3.3.3 配置响应拦截器：统一处理 `code != 0` 的错误，针对 `1002`（未登录）触发登录弹窗，针对 `1003`（权限不足）显示 Toast 提示。
    * [ ] 3.3.4 在 `frontend/src/api/` 下创建按领域拆分的 API 模块：`user.ts`、`product.ts`、`category.ts`、`tag.ts`、`home.ts`，每个导出基于 `api.md` 路径与类型签名的函数。

  * [ ] 3.4 状态管理（Pinia，`frontend/src/store/`）

    * [ ] 3.4.1 在 `store/index.ts` 中创建并导出 Pinia 实例，并在 `main.ts` 注册。
    * [ ] 3.4.2 实现 `store/user.ts`：

      * [ ] 3.4.2.1 state：`token`、`currentUser`（含 id/account/nickname/avatarUrl/wechatId/isAdmin）、`rememberMe`。
      * [ ] 3.4.2.2 actions：`login`（调用 `/users/login`，保存 token 与用户信息到 Pinia + localStorage）、`logout`、`fetchProfile`、`updateProfile`、`changePassword`。
      * [ ] 3.4.2.3 getter：`isLoggedIn`、`isAdmin`，用于路由守卫与 UI 显示。
    * [ ] 3.4.3 实现 `store/app.ts`：

      * [ ] 3.4.3.1 state：全局 loading 标记、全局错误信息、`categories`、`tags`、`productConditions`。
      * [ ] 3.4.3.2 actions：`initDictionaries` 在应用启动时调用 `GET /categories`、`GET /tags`、`GET /product-conditions` 并缓存结果。
    * [ ] 3.4.4 实现 `store/product.ts`：

      * [ ] 3.4.4.1 state：`homeRecommendations`、`homeLatest` 分页信息、最近搜索条件缓存（关键词、排序、过滤器）、当前商品详情缓存。
      * [ ] 3.4.4.2 actions：`fetchHomeData`、`searchProducts`、`fetchCategoryProducts`、`fetchMyProducts`、`fetchProductDetail`、`changeStatus` 等，调用对应 API 并更新 store。

---

* [ ] 4. 学生端通用组件开发（frontend/src/components）

  * [ ] 4.1 布局与导航组件（`components/common/`）

    * [ ] 4.1.1 `AppHeader.vue`

      * [ ] 4.1.1.1 实现固定顶部导航，左侧 Logo，中央搜索框（输入后跳转 `/search?q=xxx`），右侧“发布闲置”按钮 + 用户头像/登录按钮。
      * [ ] 4.1.1.2 搜索框支持 Enter 提交与点击搜索图标提交，保留最近搜索关键词在路由 query。
      * [ ] 4.1.1.3 未登录点击“发布闲置”时触发登录 Modal；已登录则跳转 `/products/new`。
    * [ ] 4.1.2 `AppFooter.vue`

      * [ ] 4.1.2.1 实现简洁底部，展示版权信息、项目说明链接（如 GitHub）、安全提示文案（例如“线下交易注意安全”）。

  * [ ] 4.2 通用状态组件

    * [ ] 4.2.1 `Pagination.vue`：封装分页逻辑（上一页/下一页/页码），通过 `v-model:page` 与 `pageSize` Prop 实现，支持总数展示。
    * [ ] 4.2.2 `Loading.vue`：支持两种形式：全局 loading 遮罩及局部骨架屏占位（可通过 slot 或 `type` prop 控制）。
    * [ ] 4.2.3 `Empty.vue`：接收 `title`、`description`、可选 `actionLabel`，用于列表为空时展示“暂无商品”“去发布”等提示。
    * [ ] 4.2.4 `Modal.vue`：实现通用模态框（遮罩 + 居中白卡片，支持宽度 Prop、关闭按钮、Esc 关闭），用于登录、注册、确认弹窗等。

  * [ ] 4.3 商品相关组件（`components/product/`）

    * [ ] 4.3.1 `ProductCard.vue`

      * [ ] 4.3.1.1 接收 `product: ProductCard` Prop，展示主图（1:1 / 4:3）、标题、价格、发布时间（格式化为“X天前”）、可选状态 Tag。
      * [ ] 4.3.1.2 主图使用 `mainImageUrl`，并设置 `object-fit: cover` 与 8px 圆角，悬浮时轻微上浮 + 阴影。
      * [ ] 4.3.1.3 卡片点击触发路由跳转到 `/products/:id`。
    * [ ] 4.3.2 `ProductStatus.vue`

      * [ ] 4.3.2.1 根据 `status` 显示对应胶囊 Tag：在售绿色、已售灰色、下架红色；文本内容与状态字符串匹配（`ForSale/Delisted/Sold`）。
    * [ ] 4.3.3 `ProductImageUpload.vue`

      * [ ] 4.3.3.1 使用虚线框 + “上传图片”文案；支持拖拽、点击选择多图；上传前做大小/格式校验（例如 ≤2MB、jpg/png）。
      * [ ] 4.3.3.2 本地预览缩略图，支持删除与设置主图标记（内部维护 `isPrimary` 与 `sortOrder`，表单提交时挂到 FormData）。
    * [ ] 4.3.4 `ProductForm.vue`

      * [ ] 4.3.4.1 封装发布/编辑商品表单：标题、价格、新旧程度（Radio/Select，来源 `productConditions`）、分类（Select）、标签（多选）、描述、多图上传。
      * [ ] 4.3.4.2 提供 `mode: 'create' | 'edit'` Prop 与 `initialValue`，在编辑模式下回显已有商品信息与图片。
      * [ ] 4.3.4.3 提交触发 `onSubmit` 事件，包含结构化数据 + `File[]`，由父组件调用 `POST /products` 或 `PUT /products/{id}`。
