* [ ] 1. 项目初始化与准备工作（后端整体）
  ...
  * [ ] 1.1 开发环境与基础工具准备
    ...
    * [ ] 1.1.3 安装常用 Go 开发工具：Go 扩展（VSCode/GoLand）、`gofmt` / `goimports`、`golangci-lint`。
  ...
  * [ ] 1.3 基础依赖安装（写入 `go.mod`）
    ...
    * [ ] 1.3.1 使用 `go get` 安装核心依赖：Gin、GORM、pg 驱动、JWT、bcrypt、Viper（或其他配置库）等：
  ...
  * [ ] 1.4 配置管理与环境文件
    ...
    * [ ] 1.4.1 在 `backend/config/config.go` 定义配置结构体：`ServerConfig`（端口、模式）、`DBConfig`（host、port、user、password、dbname、sslmode）、`JWTConfig`（secret、issuer、accessTokenTTL、rememberMeTTL）、`StorageConfig`（本地文件路径或对象存储信息）。
  ...
  * [ ] 1.6 代码规范与运行脚本
    ...
    * [ ] 1.6.3 在 `cmd/main.go` 实现应用入口：加载配置 → 初始化日志 → 建立 DB 连接 → 初始化路由与中间件 → 启动 HTTP 服务。
...
* [ ] 3. 后端总体架构与基础设施搭建
  ...
  * [ ] 3.1 路由总入口与模块拆分
    ...
    * [ ] 3.1.1 在 `backend/router/router.go` 定义 `SetupRouter(db *gorm.DB, cfg *config.Config) *gin.Engine`，在其中：注册通用中间件（日志、恢复、CORS）、挂载各模块路由（用户、商品、推荐、分类/标签、后台）。
...
* [ ] 5. 商品模块开发（商品主表 / 状态机 / 列表 / 搜索）
  ...
  * [ ] 5.3 服务层（`service/product/service.go`）
    ...
    * [ ] 5.3.3 `ChangeStatus(ctx, sellerID, productID, action)`：
      ...
      * 对成功的 `delist/relist` 写入内存撤销记录（键包含 productID + sellerID，TTL 3 秒）。
    * [ ] 5.3.4 `UndoLastStatusChange(ctx, sellerID, productID)`：

      * 从内存缓存读取 `fromStatus/toStatus`；
      * 若不存在或已超时，返回 `3005`；
...
* [ ] 11. 安全 / 性能 / 运维相关增强
  ...
  * [ ] 11.2 性能与缓存
    ...
    * [ ] 11.2.1 在首页推荐与最新列表中合理使用内存缓存结果，避免每次都做复杂 join；设置合适 TTL（5~10 分钟）。
