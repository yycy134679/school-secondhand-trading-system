...
2.  **管理员 (Administrator)**: 系统的维护者和监督者。
    *   **权限**: 访问后台管理系统、维护商品分类和标签库；在后台对已售商品的非状态字段进行必要的纠错/数据清洗（例如修正文案、分类或标签）。
3.  **访客 (Visitor)**: 未登录的用户。
...
2.  **已售 (Sold)**
    ...
    *   **特征**: 
        *   这是商品的终态之一,一旦进入该状态,状态字段不可逆（详见下述权限说明）。
        *   处于该状态的商品不再公开展示,不可重新上架（状态不可逆），但平台允许管理员在后台对非状态字段（如标题、描述、分类、标签等）作必要的纠错或数据清洗更新。
...
*   只有 **在售** 和 **已下架** 状态的商品可以被卖家编辑；**已售** 状态下，卖家默认不可修改该记录。
*   **已售** 商品在状态上为终态（Sold），状态不可逆；但平台可允许管理员在后台对非状态字段（例如标题、描述、分类、标签等）进行必要的纠错或数据清洗编辑（不会改变其已售事实）。
*   补充明确：**已售**为全局终态，禁止任何形式的状态变更（包括“重新上架”）；若需再次售卖，必须创建一条新的商品。平台允许管理员在后台对已售商品的非状态字段进行必要补正或数据清洗。
...
说明：表中“卖家可编辑”列为针对商品发布者（卖家）的默认行为；平台管理员可在后台对已售商品进行有限的非状态字段编辑（例如修正标题/描述、更新分类/标签、补充说明），以支持运维与纠错。普通卖家对已售商品仍默认不可编辑。
| 已下架 | ✗ | ✓ | - | ✓ |
...
5.  **业务流程**:
    ...
    4.  前端将校验通过的数据发送至后端。
    5.  后端进行基本的数据格式和唯一性校验。
    6.  后端将新用户信息存入数据库（存储 `password_hash`，只保存密码的哈希，不存明文）。
...
*   **微信联系方式（新增）**: 在个人信息模块增加“微信号（wechat_id）”字段。
    * 说明：该字段为用户级别的联系方式，用户在个人设置填写一次后，作为所有其发布商品的默认联系信息；不是每条商品记录单独设置联系方式。
    * 必填场景：若用户要发布商品，系统必须检查该用户的 `wechat_id` 是否已填写；未填写时不允许发布并需引导用户完善资料（弹窗或填写跳转）。
    * 格式提示：建议长度 4~64 字符，允许字母、数字、下划线 `_` 和短横线 `-`，前端应做初步校验，后端再次校验并拒绝非法输入。
    * 修改/频次：用户可以随时修改微信号并立即生效；为兼顾安全与滥用风险，产品可选项为限制修改频次（例如 30 天一次）；初版**不强制**此频次限制。
    * 历史/迁移兼容：若在数据迁移期间遇到历史用户没有填写微信号，平台应提示“请完善联系方式后再发布商品”；对于已存在的历史商品，在商品详情中遇到缺失的微信号时，应在“联系卖家”处显示降级文案（例如“卖家联系方式暂不可用，请稍后再试”）。
...
4.  **界面及交互**:
    *   **发布商品表单**:
        ...
        *   **商品标签**: 从预设标签列表中进行多选。
        *   **商品分类**: 从预设分类下拉菜单中进行单选。
        *   **新旧程度**: 从后端提供的枚举选项中进行单选（由 `product_conditions` 表维护，前端通过 `GET /api/v1/product-conditions` 获取）。
        *   **发布按钮**: 点击后提交表单。
5.  **业务流程**:
    1.  用户进入发布商品页面，填写所有必填信息。
    1.1 系统校验：在提交前后端须校验发布者 `wechat_id` 是否存在；若不存在则阻止发布并给出提示 '请先完善个人联系方式（微信号）'，并提供前往“个人中心”或在发布流程内弹出资料补全表单。
    2.  用户点击“发布”按钮。
    3.  后端对所有字段进行严格校验。
    4.  后端处理并存储图片。
...
*   [ ] **验收标准4**: 新发布的商品状态默认为“在售”。
    *   [ ] **验收标准5（新增）**: 发布商品前验证用户已填写微信号；若未填写，则发布被阻止并提示用户完善微信号。
...
4. **界面与交互**:
   * 列表页（“我发布的商品”）：
    ...
    * 卡片字段：主图（由图片列表中 `is_primary` 标记的图片推导）、标题、价格、当前状态（在售/已售/已下架）。
     * 操作按钮按状态动态呈现：
       * 在售：编辑、下架。
       * 已下架：编辑、重新上架。
...
5. **业务流程**:
   * A. 列表浏览：进入页面→查询该用户所有商品（含在售/已下架/已售）按发布时间倒序→展示分页→用户可搜索→动态按钮依状态呈现。
    * B. 编辑：列表点击编辑→权限与状态校验→加载编辑页面→用户修改→提交→后端字段校验与状态合法性验证→更新数据（不改状态）→返回成功→跳转回列表并刷新。已售商品默认拒绝（终态）由普通卖家编辑；管理员可在后台对已售商品的非状态字段做必要的纠错或补充信息。
...
6. **权限与状态规则（引用 1.3 状态机）**:
    * 已售为终态：状态字段不可逆（禁止 Sold -> ForSale/Delisted），不可重新上架、不可下架、不可撤销回滚至非终态；但平台允许管理员在后台对已售商品的非状态字段进行必要的纠错或数据清洗更新。
    * 可编辑：卖家仅可编辑“在售”与“已下架”状态的商品；管理员可在后台对已售商品的非状态字段进行编辑（权限控制）。
   * 状态流转允许：在售↔已下架；已售无任何流转。
   * 撤销仅适用于当前刚完成的“上/下架”操作，已售冲突或其他外部改变导致 409 时撤销失败。
7. **异常 / 分支流程**:
   * 无发布商品：显示空状态与“去发布”引导。
   * 非发布者访问编辑 / 状态操作：拒绝，提示“您无权操作该商品”。
    * 编辑已售商品：普通卖家编辑已售商品将被拒绝，提示“已售商品无法编辑”；管理员可在后台编辑已售商品的非状态字段以进行纠错或补充信息。
...
9. **验收标准（合并 & 去重）**:
    ...
    * [ ] **验收标准9**: 已售商品不出现任何状态管理按钮，且普通卖家不可编辑或重新上架；管理员可在后台编辑已售商品的非状态字段（例如标题、描述、分类与标签），但不可更改其状态。
    ...
    * [ ] **验收标准14**: 已售终态的状态字段不可被任何接口更改或撤销；后端若收到相关尝试一律拒绝。普通卖家不可编辑已售商品；管理员可在后台编辑已售商品的非状态字段用于纠错或数据清洗。
...
4.  **界面及交互**:
    ...
    *   **搜索结果页**:
        ...
        *   以商品卡片列表展示搜索结果。
        *   顶部提供筛选与排序栏：
            *   筛选：价格区间（最小价/最大价）、发布时间（如近7天/近30天，可选）。接口将提供 `publishedTimeRange` 参数，取值为 `all` / `last_7_days` / `last_30_days`，基于 `products.created_at` 进行过滤。
...
4.  **界面及交互**:
    ...
    *   **操作区**: "联系卖家"按钮。
        *   访客（未登录）：点击"联系卖家"弹出登录框，引导完成登录后再继续原动作；登录后才可查看卖家联系方式。
        *   登录用户（非卖家）：点击"联系卖家"后直接展示卖家完整微信号（wechat_id）；若卖家微信号为空（历史数据），前端展示降级文案“卖家联系方式暂不可用，请稍后再试”。
