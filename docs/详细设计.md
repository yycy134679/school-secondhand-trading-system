...
- 允许流转：
 ...
 - 终态：`Sold`，状态字段为终态且不可逆（禁止 Sold -> ForSale/Delisted）；
 - 可编辑：`ForSale` 与 `Delisted` 状态商品可被卖家编辑；管理员可在接口层权限允许的情况下，编辑已售商品的非状态字段（如标题、描述、分类、标签等），但不可更改 status 字段；普通卖家默认不可编辑已售商品。
...
| wechat_id | VARCHAR(64) | NULL | 微信号（用于联系卖家，字段为用户级，注册时可为空，发布商品时要求填写）。 |
...
- 可选索引：`INDEX idx_users_account(account)` 提升登录查询效率。
 - `wechat_id` 用途与安全：该字段作为卖家对外公开的联系方式，仅在满足权限条件（已登录且请求者 != 卖家）时返回给前端；未登录用户不返回具体微信号，前端需提示登录；若历史数据缺失，返回 null 并显示降级提示。出于隐私考虑，不建议对 `wechat_id` 建立公开索引。 
 - 可选索引：`INDEX idx_users_wechat(wechat_id)`（注意隐私，慎用，仅后台管理或必要查询时使用）。
...
| condition_id | SMALLINT | NOT NULL | 新旧程度（引用 `product_conditions.id`，接口使用 `conditionId`） |
| category_id | BIGINT | NOT NULL, FK -> categories(id) | 分类 ID |
| status | VARCHAR(16) | NOT NULL | `ForSale` / `Sold` / `Delisted` |
| main_image_url | VARCHAR(255) | NULL | 主图 URL 冗余字段，用于列表展示优化。发布/编辑/设置主图时需同步更新此字段。 |
...
注：数据库层通过触发器（例如 `trg_products_status_guard`）来约束状态字段的流转（禁止非法状态变更）。触发器仅在 `status` 字段发生变更时执行严格校验；当 `status` 不变（例如已售记录的其他字段更新）时，数据库允许更新其他字段。应用层需结合权限（普通卖家/管理员）决定是否允许对已售记录执行字段更新操作。
...
| is_primary | BOOLEAN | NOT NULL DEFAULT FALSE | 是否为主图（每个商品最多一张） |

索引：

-- `INDEX idx_product_images_product(product_id, sort_order)`。
-- 约束：每个 `product_id` 最多允许一个 `is_primary = TRUE`（可通过部分唯一索引或业务约束保证）。
...
| 3002 | 非发布者，无权操作该商品 |
| 3003 | 商品状态不允许此操作 |
| 3004 | 已售商品禁止状态变更；普通卖家不可编辑；管理员可在不变更 status 的前提下编辑非状态字段 |
| 3005 | 撤销超时或冲突（状态已变更） |
| 4001 | 分类被引用，无法删除 |
| 4002 | 标签被引用，无法删除 |
...
- 结构 `Product`：
	...
	- `Description string`
	- `Price float64`
	- `ConditionID int16` // 引用 `product_conditions.id`，在 API 层使用 `conditionId` 入参；响应可返回 `ConditionID` 和 `ConditionName`。
	- `CategoryID int64`
	- `Status string` // ForSale, Sold, Delisted
	- `MainImageURL string` // 冗余字段，同步更新，用于列表查询优化
	- `CreatedAt time.Time`
	- `UpdatedAt time.Time`
...
- 结构 `ProductImage`：
	...
	- `URL string`
	- `SortOrder int`
	- `IsPrimary bool` // 是否为主图（每个商品最多一张主图，由 DB 索引/约束保证）
...
- 接口 `ProductRepository`：
	...
	- `Update(ctx, p *Product, images []ProductImage, tagIDs []int64, isAdmin bool) error`：
		- 普通卖家仅能更新非 Sold 状态产品；管理员（isAdmin = true）可以在 status 不变的前提下更新已售 (Sold) 产品的非状态字段（例如标题/描述/分类/标签等），但禁止将 Sold 状态变更为其他状态。
...
1. `CreateProduct(ctx, sellerID int64, req CreateProductReq) (*ProductDetailDTO, error)`
	 ...
	 - 入参（业务结构）：
		 ...
		 - `CategoryID int64`
		 - `TagIDs []int64`
		 - `ConditionID int` // 入参使用 conditionId（来源于 product_conditions 表）
	 ...
	 - 处理步骤：
		 ...
		 6. 设置 `Status = ForSale`；
		 7. 设置 `MainImageURL` 为第一张图片的 URL（若有）；
		 8. 校验发布者联系方式：在写入数据库前查询 `users.wechat_id`，若为空则返回错误码（例如 `1001` / `SELLER_CONTACT_REQUIRED`），前端应提示用户去个人中心完善联系方式后重试；
		 9. 调用 `repo.Create` 完成事务插入；
		 10. 返回包含图片与标签的 DTO；

2. `UpdateProduct(ctx, actorID int64, productID int64, req UpdateProductReq, isAdmin bool) (*ProductDetailDTO, error)`

	 - 流程：
		 1. 查询商品，若不存在返回 `3001`；
		 2. 校验权限：若 `isAdmin` 为 false，则检查 `SellerID` 是否等于当前用户，否则返回 `3002`（普通用户不能编辑非自己商品）；管理员可以编辑任意商品；
		 3. 检查状态：
			 - 若商品当前状态为 `Sold` 且 `isAdmin` 为 false，则返回 `3004`（普通卖家不可编辑已售商品）；
			 - 若商品当前状态为 `Sold` 且 `isAdmin` 为 true，则仅允许在 `status` 字段未变化的前提下编辑非状态字段（标题/描述/分类/标签等）；
		 4. 按请求更新可编辑字段（标题、描述、价格、分类、标签、图片）；
		 5. 若有新图片，上传并重建 image 列表；若设置了新主图，需同步更新 `products.main_image_url`；
			  - **注意**：设置新主图前，务必先通过 `UPDATE product_images SET is_primary=false WHERE product_id=?` 重置旧主图，以避免触发唯一索引冲突。
		 6. 调用 `repo.Update(ctx, p, images, tagIDs, isAdmin)`（内部事务更新 product, images, tags，Repository层需根据 isAdmin 决定是否允许更新 Sold 商品的非状态字段）；
		 7. 返回更新后的 DTO。
...
3. `ChangeStatus(ctx, sellerID, productID int64, action string) (*ProductDTO, error)`
	 ...
	 - 逻辑：
		 - 查询商品，校验发布者；
			- 若当前状态为 Sold，直接返回错误 `3004`（Sold 状态不可变更）；
...
5. `GetProductDetail(ctx, productID int64, viewerID *int64) (*ProductDetailDTO, error)`
	 ...
	 - 逻辑：
		...
		5. `sellerWechat` 字段：
			- 返回规则：
				- 未登录用户（viewerID 为 NULL）：不返回微信号（字段为 null），前端展示“请先登录后联系卖家”。
				- 登录用户且 viewerID == sellerID（查看自己的商品）：返回完整 `wechat_id`（供个人中心查看或编辑），但在详情页不显示“联系卖家”入口。
				- 登录用户且 viewerID != sellerID：若 `users.wechat_id` 非空，返回完整微信号字符串；若为空，返回 null，前端展示降级文案“卖家联系方式暂不可用，请稍后再试”。
		 6. 推荐模块仅在 `viewerID` 非空时记录浏览行为。

6. `SearchProducts(ctx, params SearchParams) ([]ProductCardDTO, int, error)`

 	- 只查询 `status=ForSale`；
 	- 支持模糊匹配标题、描述、分类名、标签名；
 	- 支持价格区间、发布时间范围（publishedTimeRange）与排序。
`SearchParams` 字段示例：
...
- `page`, `pageSize` (int)
- `publishedTimeRange` (`all`|`last_7_days`|`last_30_days`) // 对应基于 `products.created_at` 的时间范围过滤
...
- `GET /api/v1/product-conditions`（公开）—— 新旧程度枚举列表，返回 `[{id, code, name, sortOrder}]`；
...
1. **发布商品** `POST /api/v1/products`
	 ...
	 - 字段：
		...
		- `conditionId` (number, 必填) // 新旧程度的 ID，来源于 `GET /api/v1/product-conditions` 接口
		 - `images` (file[])：多张图片
	 - Controller：
		 1. 从上下文获取 `userID`；
		 2. 解析表单字段与图片文件；
...
2. **编辑商品** `PUT /api/v1/products/:id`
	 ...
	 - JSON 请求：
		- 可选字段：`title?`, `description?`, `price?`, `categoryId?`, `tagIds?`, `conditionId?`, `imageUrls?`（重传逻辑可以简化为“覆盖式更新”）。
	 ...
	 - Query 参数：
	    ...
	    - 返回：
		    - `items: [{id, title, price, status, createdAt, mainImageUrl}]`；
		      - 注意：`mainImageUrl` 直接从 `products` 表读取。
...
7. **搜索** `GET /api/v1/products/search`
	 ...
	 - Query：
		 - `q`（关键词，可空，为空时返回全部在售）
		 - `publishedTimeRange`：`all` | `last_7_days` | `last_30_days`（可选）—— 对应基于 `products.created_at` 的时间范围过滤，默认 `all`。
	...
	过滤逻辑说明：
	- `publishedTimeRange=last_7_days` -> 仅包含 `products.created_at >= NOW() - INTERVAL '7 days'` 的记录；
	- `publishedTimeRange=last_30_days` -> 仅包含 `products.created_at >= NOW() - INTERVAL '30 days'` 的记录；
	- `publishedTimeRange=all` (默认) -> 不限制 `created_at`。

8. **分类浏览** `GET /api/v1/products/category/:categoryId`
8.1 **获取新旧程度枚举** `GET /api/v1/product-conditions`

 - Query: 无
 - Response: `[{ id: number, code: string, name: string, sortOrder: number }]`
 - 说明：该接口返回后端维护的 `product_conditions` 列表，前端用于渲染发布/编辑时的新旧程度下拉选项；前端常量 `common/constants/product_condition.ts` 应与该接口返回内容保持一致（可通过初始化加载或构建时同步）。
...
- 操作区：
	...
	- 登录且非发布者：点击“联系卖家”弹窗展示卖家 `sellerWechat` 字段（后端在 `GET /api/v1/products/:id` 返回），后端返回完整微信号；若 `sellerWechat` 为 null 则显示降级文案“卖家联系方式暂不可用，请稍后再试”。
	- 发布者本人：隐藏“联系卖家”。
...
4. `UpdateProductAsAdmin(ctx, productID int64, req AdminUpdateProductReq) (*ProductAdminDTO, error)`：
	- 描述：管理员后台更新商品信息，允许在 `status` 未变更的前提下更新 `Sold` (已售) 商品的非状态字段（用于纠错或数据清洗）；禁止通过该接口将 `Sold` 状态回退或改为其他状态。
...
- 路由示例：
	...
	- `PUT /api/v1/admin/products/:id`：管理员更新商品信息（允许在 status 未改变的前提下编辑已售商品的非状态字段）。
	- 分类/标签维护接口见上一节。
...
| /api/v1/product-conditions | GET | 新旧程度枚举 | 否 |
...
- `sellerWechat` (string | null)：
	- 未登录用户：返回 null；前端提示“请先登录后联系卖家”；
	- 登录且为卖家本人：服务端可返回完整微信号（用于个人中心或编辑），但在详情页不显示“联系卖家”入口；
	- 登录且非卖家：若卖家已填写微信号返回完整微信号；若未填写返回 null 并展示降级文案“卖家联系方式暂不可用，请稍后再试”。
...
6. 已售商品的状态字段禁止回退或变更（Sold 不可逆），相关尝试一律返回错误；普通卖家不可编辑已售商品；管理员可在后台编辑已售商品的非状态字段（如标题、描述、分类、标签），但禁止修改 status 字段。
